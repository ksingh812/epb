			...
			if ( $listings->have_posts() ) : ?>
				<div class="grid list">
					<?php while ( $listings->have_posts() ) :
						...
						<a href="<?php the_job_permalink(); ?>" class="grid__item">
							<article class="card  card--listing" data-latitude="<?php echo get_post_meta( $post->ID, 'geolocation_lat', true ); ?>"
								...
								<aside class="card__image" style="background-image: url(<?php echo listable_get_post_image_src( $post->ID, 'listable-card-image' ); ?>);">
									...
									if ( $job_manager_bookmarks !== null && method_exists( $job_manager_bookmarks, 'is_bookmarked' ) ) {
										...
										<div class="heart <?php echo $bookmark_state; ?>"></div>
									<?php } ?>
...
class Front_Page_Spotlights_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		<h3 class="widget_title">
			...
			if ( ! empty( $widget_subtitle ) ) { ?>
				<span class="widget_subtitle"><?php echo $widget_subtitle; ?></span>
			<?php } ?>
		</h3>
		...
		<div class="grid">
			<?php foreach ( $spotlights as $key => $spotlight ) : ?>
				<div class="grid__item">
					<div class="card  card--post  card--feature">
						<div class="card__content">
	...
	public function form( $instance ) {
		...
		$widget_subtitle = esc_attr( $instance['widget_subtitle'] );
		//if the user is just creating the widget ($original_instance is empty)
					...
					foreach ( $spotlights as $key => $spotlight ) :
						...
						<div class="group_panel_widget_spotlight" data-count="<?php echo $key; ?>">
							...
							<p>
								...
								<input data-field="title" type="text" value="<?php echo $spotlight->title; ?>">
							</p>

							<p>
								<label for="desc"><?php esc_html_e( 'Content:', 'listable' ) ?></label>
								<br/>
								<textarea data-field="desc" cols="20" rows="5"><?php echo $spotlight->desc; ?></textarea>
							</p>
						</div>
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['spotlights_value']    = strip_tags( $new_instance['spotlights_value'] );
