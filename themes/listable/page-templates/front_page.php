					...
					<header class="entry-header has-image">
						<div class="entry-featured"<?php if ( ! empty( $has_image ) ) {
							...
							<?php if ( ! empty( $the_random_hero ) && property_exists( $the_random_hero, 'post_mime_type' ) && strpos( $the_random_hero->post_mime_type, 'video' ) !== false ) {
								...
								if ( has_post_thumbnail($the_random_hero->ID) ) {
									$image = wp_get_attachment_url( get_post_thumbnail_id($the_random_hero->ID, 'full'));
									$poster = ' poster="' . $image . '" ';
								} else {
									$poster = ' ';
								}
						...
						<div class="header-content">
							...
							<form class="search-form  js-search-form" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>" role="search">
								...
								?>
								<div class="search-field-wrapper<?php echo $has_search_menu ? '  has--menu' : ''; ?>">
									<input class="search-field  js-search-suggestions-field" type="text" name="s" id="s" placeholder="<?php esc_html_e( 'What are you looking for?', 'listable' ); ?>" autocomplete="off" value="<?php the_search_query(); ?>"/>
